...
class SimpsonNumericIntergral(NumericIntegral):
    ...
    def integrate(self):
        ...
        for i in range(1, self._sub_intervals + 1):
            res += (self._function(self._nodes[i-1]) + 4 * self._function((self._nodes[i - 1] + self._nodes[i])/2) + self._function(self._nodes[i]))
        result *= (self._interval.width) / (6 * self._sub_intervals)
        return result