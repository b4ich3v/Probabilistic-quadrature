...
class NumericIntegralFactory(NumericIntegralAbstractFactory):
    def create(self, creation_pattern: NumericIntegrationPattern, *args, **kwargs) -> NumericIntegral:
        ...
        if creation_pattern == NumericIntegrationPattern.LEGENDRE:
            return GaussLegendreIntegral(*args, **kwargs)
        if creation_pattern == NumericIntegrationPattern.HERMITE:
            return GaussHermiteIntegral(*args, **kwargs)
        if creation_pattern == NumericIntegrationPattern.LAGUERRE:
            return GaussLaguerreIntegral(*args, **kwargs)
        if creation_pattern == NumericIntegrationPattern.CHEBYSHEV:
